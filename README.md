...
- **Qwen3.5-0.8B** (hybrid architecture): drop the
  ...
  **MTP (multi-token prediction) — architectural analysis, not enabled:**
  Qwen3.5 ships a trained NextN head (confirmed in the checkpoint:
  `mtp.fc` [1024,2048] input projection + `mtp.layers.0` full-attention
  layer) and this llama.cpp fork supports `--spec-type draft-mtp`.
  An m=32 verification engine set was built and staged
  (`gemm/verify-m32/`, 24 layers + post, BF16) to test the economics.

  **Why MTP is a net loss on this architecture** — the complete chain:

  1. Speculative decoding's payoff is verifying k+1 tokens for ≈1 pass.
     On this card, one pass costs ~37 ms (weight-stream-bound), so the
     marginal cost of extra tokens is nearly zero *if the engine has the
     right shape*. The vendor engines only offer m=1 and m=128.

  2. An m=4 build was attempted: the toolchain rejected it —
     `linear_attn chunk_size: 32` is the minimum. The 18 gated-delta-net
     layers update state sequentially within each chunk; the NPU compiler
     is hardwired for 32-wide chains. This is a silicon constraint.

  3. With m=32 (the minimum), the verify cost is ~197 ms vs 37 ms for
     sequential decode. The economics table:

     | k (draft) | cycle cost | acceptance | tokens/cycle | effective t/s |
     |---|---|---|---|---|
     | 3 | ~203 ms | 70% | 3.1 | 15 t/s (44% **loss**) |
     | 7 | ~211 ms | 55% | 4.9 | 23 t/s (15% loss) |
     | 15 | ~227 ms | 40% | 7.0 | 31 t/s (break-even) |
     | 31 | ~259 ms | 25% | 8.8 | 34 t/s (26% gain, optimistic) |

  4. Even at the best case (k=31 with 25% deep-tree acceptance), the
     gain is ~26% and depends on optimistic acceptance rates at depth.

  **The architectural trade-off**: the hybrid linear-attention design
  that gives Qwen3.5 its flat 27 t/s decode (fixed recurrent state, no
  KV growth) is the *same property* that prevents efficient small-batch
  verification. Dense-attention models (like the 0.6B) can build m=4
  engines and would benefit from MTP; hybrid models on this NPU cannot.
  This is a genuine hardware-architecture interaction, not a fixable
  bug.

  **What's staged for the future** (if a denser-attention model or a
  chip with sub-32-token linear-attention chunks arrives):
  - m=32 verify engine set: `gemm/verify-m32/` (buildable to s4 with a
    GPTQ checkpoint)
  - MTP head weights confirmed extractable from the checkpoint
  - llama.cpp `--spec-type draft-mtp` path identified in the fork
  - Multi-row head engine (vocab64 pattern) is precedent for batched
    logits
